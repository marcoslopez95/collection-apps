...
class FingerprintAuthentication extends StatefulWidget {
  static String tag = '/finger_print_authentication';
  final bool isDirect;

  FingerprintAuthentication({this.isDirect = false});
...
class _FingerprintAuthenticationState extends State<FingerprintAuthentication> {
    ...
    try {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return HandleError(code: e.code, message: e.message);
        },
      );
    }
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: appBar(context, 'Fingerprint Authentication', isDashboard: widget.isDirect),
        body: ListView(
          shrinkWrap: true,
