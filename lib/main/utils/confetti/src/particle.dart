...
class ParticleSystem extends ChangeNotifier {
  ...
  List<Particle> _generateParticles({int number = 1}) {
    return List<Particle>.generate(number, (i) => Particle(_generateParticleForce(), _randomColor(), _randomSize(), _gravity, _particleDrag));
  }
...
class Particle {
  Particle(vmath.Vector2 startUpForce, Color color, Size size, double gravity, double particleDrag)
