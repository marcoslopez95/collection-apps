import 'package:access_maketicket/themes/theme11/model/T11Model.dart';
import 'package:access_maketicket/themes/theme11/utils/T11Images.dart';

List<Music> generateAlbumMusic() {
  List<Music> album = [];
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  return album;
}

List<Music> musicList1() {
  List<Music> album = [];
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));

  return album;
}

List<Music> musicList2() {
  List<Music> album = [];
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer5, 'Hey Jude', 'The Beatles'));
  album.add(Music.name(t11_ic_singer3, 'Billie Jean', 'Michael Jackson'));
  album.add(Music.name(t11_ic_singer1, 'Imagine', 'John Lennon'));
  album.add(Music.name(t11_ic_singer2, 'One', 'U2'));
  album.add(Music.name(t11_ic_singer4, 'Hey Jude', 'The Beatles'));
  return album;
}

List<Music> generateWalkThroughMusic() {
  List<Music> album = [];
  album.add(Music.name("images/theme11/t11_walkthrough_1.png", 'Music is Life', 'let\'s start listening you favourite music.'));
  album.add(Music.name('images/theme11/t11_walkthrough_2.png', 'Music Expresses', 'let\'s start listening you favourite music.'));
  album.add(Music.name('images/theme11/t11_walkthrough_3.png', 'Music is Dream', 'let\'s start listening you favourite music.'));
  return album;
}
